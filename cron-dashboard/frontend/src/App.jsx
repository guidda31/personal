    ...
    <div style={{ fontFamily: 'Inter,system-ui,sans-serif', background: '#0f172a', minHeight: '100vh', color: '#e5e7eb', display: 'grid', gridTemplateColumns: '220px 1fr' }}>
      ...
      <main style={{ padding: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          ...
          <button onClick={loadSummaryAndJobs} style={{ ...box, cursor: 'pointer', color: '#e5e7eb' }}>
            새로고침
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: 12 }}>
          ...
          <div style={box}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              <button onClick={() => setTab('info')} style={{ ...box, padding: '6px 10px', background: tab === 'info' ? '#0b2536' : '#111827', color: '#e5e7eb', cursor: 'pointer' }}>
                기본정보
              </button>
              <button onClick={() => setTab('runs')} style={{ ...box, padding: '6px 10px', background: tab === 'runs' ? '#0b2536' : '#111827', color: '#e5e7eb', cursor: 'pointer' }}>
                실행이력
              </button>
              <button onClick={() => setTab('raw')} style={{ ...box, padding: '6px 10px', background: tab === 'raw' ? '#0b2536' : '#111827', color: '#e5e7eb', cursor: 'pointer' }}>
                Raw JSON
              </button>
            </div>
            ...
            {tab === 'info' && detail && (
              <div style={{ fontSize: 13, lineHeight: 1.7 }}>
                ...
                <div>
                  Schedule: {detail.scheduleExpr} @ {detail.scheduleTz}
                </div>
                <div>
                  Next / Last: {fmt(detail.nextRunAtMs)} / {fmt(detail.lastRunAtMs)}
                </div>
                <div>
                  Delivery: {detail.deliveryMode || '-'} / {detail.deliveryChannel || '-'} / {detail.deliveryTo || '-'}
                </div>
              ...
              <div>
                {runs.map((r) => (
                  <div key={r.runId} style={{ borderTop: '1px solid #263142', padding: '8px 0', fontSize: 12 }}>
                    <div>
                      <b>{fmt(r.runAtMs)}</b> · {r.status}
                    </div>
                    <div style={{ color: '#94a3b8' }}>
                      duration: {r.durationMs ?? '-'} ms · delivery: {r.deliveryStatus || '-'} · tokens: {r.totalTokens ?? '-'}
                    </div>
                  </div>
                ))}
              </div>
